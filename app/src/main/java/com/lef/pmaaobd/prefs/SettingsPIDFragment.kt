...
import com.lef.pmaaobd.datastore.MaxControl
import com.lef.pmaaobd.datastore.Screen
import com.lef.pmaaobd.stats.R
import com.lef.pmaaobd.stats.ObdServiceWrapper
...
class SettingsPIDFragment:  PreferenceFragmentCompat() {
    ...
    var obdService: ObdServiceWrapper? = null

    var obdConnection = object : ServiceConnection {
        override fun onServiceConnected(className: ComponentName, service: IBinder) {
            obdService = (service as ObdServiceWrapper.LocalBinder).getService()
            obdService!!.loadPidInformation(false) {
                pids ->
                requireActivity().runOnUiThread {
                    val valuesQuery = pids.map { "obd_${it.first}" }.toTypedArray()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        pidPref.setOnPreferenceChangeListener { _, newValue ->
            if (newValue == "") {
                ...
                val pidOnly = (newValue as String).substring("obd_".length)
                obdService!!.pids!!.firstOrNull {
                    it.first == pidOnly
                }?.let {entryVal ->
                    // PID info format: name,shortName,unit,max,min
                    var maxVal = ceil(entryVal.second[3].toDouble()).toInt()
                    var minVal = floor(entryVal.second[4].toDouble()).toInt()
                    if (minVal == 0 && maxVal == 0) {
                        maxVal = 100
                    }
                    labelPref.text = entryVal.second[1]
                    minValuePref.text = minVal.toString()
                    maxValuePref.text = maxVal.toString()
        ...
        mBound = ObdServiceWrapper.runStartIntent(requireContext(), obdConnection)
        ...
        lifecycleScope.launch {
            requireContext().dataStore.data.map {
                ...
                if (pidPref.value.startsWith("obd")) {
                    enableItems(true)
                }
            }
        }
    ...
    override fun onDestroy() {
        ...
        if (mBound) {
            try {
                requireContext().unbindService(obdConnection)
