...
class SettingsDashboard : PreferenceFragmentCompat() {
    ...
    var obdConnection = object : ServiceConnection {
        override fun onServiceConnected(className: ComponentName, service: IBinder) {
            mBound = true
            val obdService = (service as ObdServiceWrapper.LocalBinder).getService()
            obdService.loadPidInformation(false) { pids ->
                activity?.let {
                    it.runOnUiThread {
                        mainCat.title = null
    ...
    override fun onDestroy() {
        ...
        if (mBound) {
            try {
                requireActivity().unbindService(obdConnection)
