...
import com.lef.pmaaobd.stats.databinding.FragmentChartBinding
...
class ObdChart: Fragment() {
    ...
    private val series = HashMap<ObdData, LineGraphSeries<DataPointInterface>>()
    ...
    fun setupItems(obdData: Array<ObdData>) {
        graph.series.clear()
        series.clear()

        val defaultColors = resources.obtainTypedArray(R.array.chartColors)
        val colors = obdData.mapIndexed { index, td ->
        ...
        legendBinding = obdData.mapIndexed { idx, data ->
    ...
    fun notifyUpdate(data: ObdData, binding: LegendBinding) {
